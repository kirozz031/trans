...
export default fp(async (fastify: any) => {
  // Export my data
  fastify.get('/api/me/export', { preHandler: fastify.requireAuth }, async (req: any) => {
  ...
  fastify.post('/api/me/anonymize', { preHandler: fastify.requireAuth }, async (req: any) => {
  ...
  fastify.delete('/api/me', { preHandler: fastify.requireAuth }, async (req: any) => {
    await prisma.user.update({
      where: { id: req.user.id },
